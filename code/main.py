...
ball_lower_limits = np.array(
    [data["ball"]["h_min"], data["ball"]["s_min"], data["ball"]["v_min"]])
ball_upper_limits = np.array(
    [data["ball"]["h_max"], data["ball"]["s_max"], data["ball"]["v_max"]])

blue_lower_limits = np.array(
    [data["bluegoal"]["h_min"], data["bluegoal"]["s_min"], data["bluegoal"]["v_min"]])
blue_upper_limits = np.array(
    [data["bluegoal"]["h_max"], data["bluegoal"]["s_max"], data["bluegoal"]["v_max"]])

pink_lower_limits = np.array(
    [data["pinkgoal"]["h_min"], data["pinkgoal"]["s_min"], data["pinkgoal"]["v_min"]])
pink_upper_limits = np.array(
    [data["pinkgoal"]["h_max"], data["pinkgoal"]["s_max"], data["pinkgoal"]["v_max"]])

line_lower_limits = np.array(
    [data["line"]["h_min"], data["line"]["s_min"], data["line"]["v_min"]])
line_upper_limits = np.array(
    [data["line"]["h_max"], data["line"]["s_max"], data["line"]["v_max"]])
...
def drive_to(time_passed, mapped_speed, rad):
    ...
    send_command(time_passed, "forward_adjust",
                 (mapped_speed-0.8) * (rad/2), int(speed))
    movement_action = "drive_to | " + "precentage: " + \
        str(round(precentage)) + "% | rad: " + str(round(rad))
...
def rotate(time_passed, rad, x, frame, mapped_speed):
    ...
    if keypoints != []:
        ...
        movement_action += " | goal: " + \
            str(round(x[0])) + " | rad: " + str(rad)
...
# Keypoint detection
    ...
    keypoints, erra = cv2.findContours(
